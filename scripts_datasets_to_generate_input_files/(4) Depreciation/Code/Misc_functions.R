...
################################################################################
ImanConover = function(Y,sigma,Icor=F,seedval=NA) {
  ...
  ############################################
  Z=matrix(rnorm(nr*nc),nr,nc)
  Z=(Z%*%solve(chol(cov(Z))))%*%(chol(sigma))
  for(j in 1:nc) {
    Z[,j]=Z[,j]-mean(Z[,j])
    ys=sort(Y[,j])
    Y[,j] = ys[rank(Z[,j],ties.method="first")]
  }
  Y
} # end function ImanConover
################################################################################
ImanConoverYcZc=function(Y,sigma,Icor=F,seedval=1001) {
  if(ncol(Y)!=ncol(sigma)) stop('Dimensions of Y and sigma do not match')
  if(!is.na(seedval)) set.seed(seedval)
  nc=ncol(Y)
  nr=nrow(Y)
  ...
  for(j in 1:nc) {
    Zc[,j]=Zc[,j]-mean(Zc[,j])
    ys=sort(Y[,j])
    Y[,j] = ys[rank(Zc[,j],ties.method="first")]
  }
...
################################################################################
FactorYcZc = function(Y,rhos=0.5,seedval=NA) {
  nr=nrow(Y); nc=ncol(Y); nrhos=length(rhos)
  if(nrhos>1 & nrhos!=nc) stop('Dimensions of Y and rhos do not match') 
  if(nrhos==1) rhos = rep(rhos,nc)
  if(!is.na(seedval)) set.seed(seedval)
  Yc=Y
  zF = rnorm(nr)
  Zc = matrix(0,nr,nc)
  for(j in 1:nc){
   Zc[,j] = rhos[j] * zF  + sqrt(1-rhos[j]^2) * rnorm(nr)  
   ys=sort(Y[,j])
   Yc[,j] = ys[rank(Zc[,j],ties.method="first")]
...
###############################################################################
PSD=function(sigma,Icor=F,eps=1E-5){
 tmp=eigen(sigma)
 v=tmp$vectors
 (d=tmp$values)
 d[d<=eps]=eps
...
queryRData=function(fname,summary=F){
  myfun2=function(fname,summary=F){
    ...
    if(summary==T){
     ...
     (obj_summary =  sapply(obj3,summary))
...
ecop=function(x,y,col = 1,pch = 1,cex = 1) {
  ...
  if (xfirst == T)
    points(myrank(x),myrank(y),pch = pch,cex = cex,col = col)
  if (xfirst != T)
    points(myrank(y),myrank(x),pch = pch,cex = cex,col = col)
} # end function ecop
##################################
ecop2=function(x,y,col = 1,pch = 1,cex = 1) {
  ...
  x2=x[myrank(y)<0.5];y2=y[myrank(y)<0.5]
  ...
  x3=x[myrank(y)>0.5];y3=y[myrank(y)>0.5]
...
#rholist[10]=-0.9
#(R=jcode64::rho2cor(rholist))
#eigen(R)$values
#tmp=jcode64::ImanConoverYcZc(X,R)
#cov2cor(tmp$sigma)
#Y=tmp$Yc
#Y[1,]=c(1,-1,1,-1,1)*5
#Y[2,]=c(1,-1,1,-1,1)*-10
...
#
#my_x11()
#pairs(Y,pch=20,cex=2,diag.panel = panel.hist)
#
###############################################################################
# Set plotting parameters in global
# work space for use in ecop2
#xfirst = F
###############################################################################
#my_x11()
#pairs(Y, lower.panel = ecop,pch=20,cex=2,col=colors,main='ecop')
###############################################################################
#my_x11()
#pairs(Y, lower.panel = ecop2,pch=20,cex=2,col=colors,main='ecop2')
###############################################################################
#my_x11()
#pairs(Y, lower.panel = ecop3,pch=20,cex=2,col=colors,main='ecop3')
################################################################################
#my_x11()
#pairs(Y, lower.panel = ecop2,pch=20,cex=2,diag.panel=panel.hist,main='ecop2')
######################################################################
#my_x11()
#pairs(Y, lower.panel = ecop3,pch=20,cex=2,diag.panel=panel.hist1,main='ecop2')
###############################################################################
###############################################################################
# end functions for specialized pairs plots
###############################################################################
...
###############################################################################
my_HCOP=function(YSIM,Y,sscale=0.20,YPROB=NULL,seedval=NA){
###############################################################################
...
###############################################################################
# put smoothed marginals in HCOP rank order
###############################################################################
j=1
for(j in 1:nc){
 y=sort(YSIM[,j])
 YSIM[,j]=y[rank(HCOP[,j])] 
}
YSIM$index=HCOP$index
###############################################################################
as.data.frame(YSIM)
###############################################################################
} # end function HCOP
...
###############################################################################
LPSOLVER=function(LP,solver='clp',printobj=F){
...
# if('rnames' %in% names(LP) == F) LP$rnames = LP$rnames   
 ...
 if('solution' %in% names(LP) == T) solution=LP$solution 
  ...
  dirs=LP$dirs
  if('rhs' %in% names(LP) == F) rhs = LP$rhs
  rlower=LP$rlower
  rupper=LP$rupper
  ...
  if(exists('rhs'))      M=cbind(M,dirs,'|',rhs,'|')
  if(exists('rlower'))    M=cbind(M,rlower,'|',rupper,'|')

  (tmp=c(cnames,'|','LHS','|'))

      
  if(exists('rhs'))    (tmp=c(tmp,'dirs','|','rhs','|'))
  ...
  (tmp=c('',obj,'|','lhsOBJ','=',paste(LPsense)))
